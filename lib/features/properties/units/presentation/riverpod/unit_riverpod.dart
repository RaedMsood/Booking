...
final selectedOfferUnitsSectionIdProvider =
    StateProvider.family<int?, int>((ref, propertyId) => null);
...
final getAllOfferUnitsProvider = StateNotifierProvider.family<
    GetAllOfferUnitsNotifier,
    DataState<UnitsInPropertySectionsModel>,
    Tuple2<int, int>>(
  (ref, params) {
    final int propertyId = params.value1;
    final int sectionId = params.value2;
    return GetAllOfferUnitsNotifier(propertyId, sectionId);
  },
);

class GetAllOfferUnitsNotifier
    extends StateNotifier<DataState<UnitsInPropertySectionsModel>> {
  final int propertyId;
  final int sectionId;

  GetAllOfferUnitsNotifier(this.propertyId, this.sectionId)
      : super(
          DataState<UnitsInPropertySectionsModel>.initial(
            UnitsInPropertySectionsModel.empty(),
          ),
        ) {
    getData();
  }
  ...
  Future<void> getData({
    ...
    final result = await _controller.getAllOfferUnits(
      page: nextPage,
      propertyId: propertyId,
      sectionId: sectionId,
    );
    ...
    result.fold(
      ...
      (data) {
        ...
        if (oldData.units.data.isEmpty || isRefresh || !moreData) {
          ...
          final PaginationModel<UnitsModel> units = oldData.units.copyWith(
            data: [...oldData.units.data, ...data.units.data],
            currentPage: data.units.currentPage,
            lastPage: data.units.lastPage,
          );
